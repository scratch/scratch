...
import { validateProjectName, validateNamespaceForUser, getEmailDomain, GLOBAL_NAMESPACE } from '../shared/project'
...
// ============================================================================
// Namespace (User Config default + Project Config)
// ============================================================================

/**
 * Format namespace for display (shows 'global' as 'global (shared namespace)').
 */
export function formatNamespace(namespace: string): string {
  return namespace === GLOBAL_NAMESPACE ? 'global (shared namespace)' : namespace
}

/**
 * Prompt for namespace selection.
 * Offers user's email domain (if available) and global namespace.
 *
 * @param userEmail - User's email address (to derive domain option)
 * @param currentValue - Current configured value (if any)
 * @param globalDefault - Global default from user config (if any)
...
/**
 * Prompt for namespace as a URL choice (for deploy flow).
 * Shows full URLs to help user understand the impact.
 *
 * @param userEmail - User's email address (to derive domain option)
 * @param projectName - Project name for URL display
 * @param pagesUrl - Base pages URL
 * @param currentValue - Current configured value (if any)
 * @returns The selected namespace
 */
export async function promptNamespaceAsUrl(
  userEmail: string,
  projectName: string,
  pagesUrl: string,
  currentValue?: string
  ...
  // Default to domain unless explicitly set to global
  const defaultNs = currentValue === GLOBAL_NAMESPACE ? GLOBAL_NAMESPACE : userDomain

  return select('Choose your project URL:', choices, defaultNs)
}
...
export { GLOBAL_NAMESPACE } from '../shared/project'
